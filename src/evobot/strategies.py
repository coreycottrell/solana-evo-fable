"""Pure signal functions of (prices, genome) — four theses."""
...
def signal_mean_reversion(prices: Sequence[float], params: Genome) -> Signal:
...
def signal_momentum(prices: Sequence[float], params: Genome) -> Signal:
...
def signal_breakout(prices: Sequence[float], params: Genome) -> Signal:
    lookback = max(5, params.lookback)
    if len(prices) < lookback + 1:
        return Signal(action=SignalAction.HOLD, reason="warmup")
...
def signal_liquidity_fade(prices: Sequence[float], params: Genome) -> Signal:
...
def generate_signal(org: Organism, prices: Sequence[float], now: datetime) -> Signal:
    ...
    if not (org.genome.vol_filter_min <= v <= org.genome.vol_filter_max):
        if org.position.qty != 0:
            # still allow exits via stops/max_hold above; hold for opens
            pass
        else:
            return Signal(action=SignalAction.HOLD, reason="vol_filter")
    fn = SIGNAL_FN[org.thesis_type]
    sig = fn(prices, org.genome)
    if org.position.qty != 0:
        # only act if signal is a close or hold
